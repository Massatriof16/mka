...
chmod a+x ${current_directory}/TWRP_${Device_Name}_vendor_boot.xz
if [ -z "${id_chat}" ]; then
echo " id chat Tidak diatur, Melewati kirim notifikasi !"
else
curl -X POST "https://api.telegram.org/bot${Token}/sendMessage" -d "chat_id=${id_chat}&text= NEW BUILD TWRP_${Device_Name}!"
curl -F document=@"${current_directory}/TWRP_${Device_Name}_${Build_Target}.xz" https://api.telegram.org/bot${Token}/sendDocument?chat_id=${id_chat}
fi
else
chmod a+x ${current_directory}/TWRP_${Device_Name}_${Build_Target}.xz
if [ -z "${id_chat}" ]; then
echo " id chat Tidak diatur, Melewati kirim notifikasi !"
else
...
curl -F document=@"${current_directory}/TWRP_${Device_Name}_${Build_Target}.xz" https://api.telegram.org/bot6788930639:AAHpp3siVn8wnWp3SGOM_uC2EDFaXWjyE6I/sendDocument?chat_id=6561499315
fi
fi
