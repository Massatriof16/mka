...
echo " "
  ...
  echo "$Build_Status Gagal"
       ...
       git clone ${Common} -b ${Branch_dt_twrp} ${Path_Common}
        ...
        if [ "$Build_Status" = "PBRP" ]; then
        export ALLOW_MISSING_DEPENDENCIES=true; . build/envsetup.sh; lunch pb_${Lunch}-eng; make ${Build_Target}image
        else
...
curl -T "${current_directory}/${Build_Status}_${Device_Name}_vendor_boot.img.xz" -u:${api} https://pixeldrain.com/api/file/
else
echo " "
chmod a+x ${current_directory}/TWRP_${Device_Name}_${Build_Target}.img.xz
curl -T "${current_directory}/${Build_Status}_${Device_Name}_${Build_Target}.img.xz" -u:${api} https://pixeldrain.com/api/file/
fi
fi
...
echo " "
echo " "
curl -X POST "https://api.telegram.org/bot${Token}/sendMessage" -d "chat_id=${id_chat}&text=Melakukan Repo sync Minimal Manifest ${Build_Status} ${Manifest_branch}"
echo " "
fi
...
echo " "
curl -F document=@"${current_directory}/${Build_Status}_${Device_Name}_vendor_boot.img.xz" https://api.telegram.org/bot${Token}/sendDocument?chat_id=${id_chat}
echo " "
else
chmod a+x ${current_directory}/TWRP_${Device_Name}_${Build_Target}.img.xz
echo " "
curl -X POST "https://api.telegram.org/bot${Token}/sendMessage" -d "chat_id=${id_chat}&text= Sukses Build ${Build_Status}_${Device_Name}!"
echo " "
curl -F document=@"${current_directory}/${Build_Status}_${Device_Name}_${Build_Target}.img.xz" https://api.telegram.org/bot${Token}/sendDocument?chat_id=${id_chat}
echo " "
fi
fi
...
chmod a+x ${current_directory}/${Build_Status}_${Device_Name}_${Build_Target}.img.xz
echo " "
curl -F "chat_id=${id_chat}" \
  -F "message_thread_id=${id_topic}" \
  -F "text=SUKSES BUILD ${Build_Status} ${Device_Name}" \
...
curl -F "chat_id=${id_chat}" \
  -F "message_thread_id=${id_topic}" \
  -F "document=@${current_directory}/${Build_Status}_${Device_Name}_${Build_Target}.img.xz" \
 https://api.telegram.org/bot${Token}/sendDocument
echo " "
fi
